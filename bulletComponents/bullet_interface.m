classdef bullet_interface < handle
  ...
  methods
    ...
    function this = bullet_interface(varargin)
      this.bulletHandle = buckshot('new', varargin{:});
      this.Terrain = [];
    ...
    function delete(this)
      buckshot('delete', this.bulletHandle);
    end
    ...
    function AddRaycastVehicles( this, RayVehicles  )
      ...
      for i=1:numel(RayVehicles),
        ...
        id = buckshot('AddRaycastVehicle', this.bulletHandle, ...
    ...
    function [ids] = AddConstraints(this, Constraints)
      ...
      for i=1:numel(Constraints),
        ...
        if strcmp(type, 'PointToPoint_one')==true,
          ...
          id = buckshot('AddConstraint', this.bulletHandle, ...
    ...
    function CommandCompound(this, Compound)
      if isa(Compound, 'Vehicle'),
        ...
        id = Compound.GetID();
        buckshot('CommandCompound', this.bulletHandle, 'Vehicle', ...
          id, steering, force);
      end
    end
    ...
    function CommandRaycastVehicle(this, Vehicle)
      if this.gui.draw,
        ...
        id = Vehicle.GetID();
        buckshot('CommandRaycastVehicle', this.bulletHandle, ...
    ...
    function [steering, force, lin_vel, ang_vel] = GetMotionState(this, Vehicle)
      ...
      [steering, force, lin_vel, ang_vel] = ...
        buckshot('GetMotionState', this.bulletHandle, id);
    end
    
    function SetToGround(this, Vehicle, x_coord, y_coord)
      id = Vehicle.GetID();
      % Get our new position
      [position] = buckshot('SetToGround', this.bulletHandle, id, ...
        x_coord, y_coord);
    ...
    function ResetVehicle(this, Vehicle, start_pose, start_rot)
      id = Vehicle.GetID();
      buckshot('ResetVehicle', this.bulletHandle, id, start_pose, start_rot);
    end
    ...
    function StepSimulation(this)
      buckshot('StepSimulation', this.bulletHandle);
      for i = 1:numel(this.Shapes),
