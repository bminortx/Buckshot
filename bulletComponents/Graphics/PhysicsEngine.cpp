...
bool PhysicsEngine::Init(double dGravity, double dTimeStep,
  ...
  boost::shared_ptr<btDbvtBroadphase> Broadphase(new btDbvtBroadphase);
  bt_broadphase_ = Broadphase;

  boost::shared_ptr<btSequentialImpulseConstraintSolver> Solver(
      new btSequentialImpulseConstraintSolver);
  bt_solver_ = Solver;
  boost::shared_ptr<btDiscreteDynamicsWorld> DWorld(
  ...
  vehicle_raycaster_ =
      boost::make_shared<btDefaultVehicleRaycaster>(dynamics_world_.get());
  return true;
}
...
void PhysicsEngine::RegisterObject(const boost::shared_ptr<ModelNode>& pItem) {

  /*********************************************************************
   *ADDING SHAPES
   **********************************************************************/
  if (boost::shared_ptr<Shape> pNodeShape =
      boost::dynamic_pointer_cast<Shape>(pItem)) {

    /************************************
     *ADDING A RAYCAST VEHICLE
     ************************************/

    if (boost::dynamic_pointer_cast<SimRaycastVehicle>(pNodeShape) != NULL) {
    ...
    //  Box
    if (boost::dynamic_pointer_cast<BoxShape>(pNodeShape) != NULL) {
    ...
    //  Cylinder
    else if (boost::dynamic_pointer_cast<CylinderShape>(pNodeShape) != NULL) {
    ...
    //  Plane
    else if (boost::dynamic_pointer_cast<PlaneShape>(pNodeShape) != NULL) {
    ...
    //  Heightmap
    else if (boost::dynamic_pointer_cast<HeightmapShape>(pNodeShape) != NULL) {
    ...
    //  Mesh
    else if (boost::dynamic_pointer_cast<MeshShape>(pNodeShape) != NULL) {
   ...
   **********************************************************************/
  ...
  else if (boost::dynamic_pointer_cast<Constraint>(pItem) != NULL) {
    ...
    if (dynamic_cast<PToPOne*>(pNodeCon) != NULL) {
      ...
      if (isVehicle(pCon->m_Shape_A)) {
        boost::shared_ptr<Vehicle_Entity> Shape_A =
            ray_vehicles_map_.at(pCon->m_Shape_A);
        RigidShape_A = Shape_A->rigid_body_.get();
      }
      else {
        boost::shared_ptr<Entity> Shape_A = shapes_map_.at(pCon->m_Shape_A);
        RigidShape_A = Shape_A->rigid_body_.get();
      }
      ...
      if (isVehicle(pCon->m_Shape_B)) {
        boost::shared_ptr<Vehicle_Entity> Shape_B =
            ray_vehicles_map_.at(pCon->m_Shape_B);
        RigidShape_B = Shape_B->rigid_body_.get();
      }
      else {
        boost::shared_ptr<Entity> Shape_B = shapes_map_.at(pCon->m_Shape_B);
        RigidShape_B = Shape_B->rigid_body_.get();
      }
      btVector3 pivot_A(pCon->m_pivot_in_A[0], pCon->m_pivot_in_A[1],
                        pCon->m_pivot_in_A[2]);
    ...
    //  Hinge
      ...
      btRigidBody* RigidShape_B;
      if (isVehicle(pCon->m_Shape_A)) {
        boost::shared_ptr<Vehicle_Entity> Shape_A = ray_vehicles_map_.at(pCon->m_Shape_A);
        RigidShape_A = Shape_A->rigid_body_.get();
      }
      else {
        boost::shared_ptr<Entity> Shape_A = shapes_map_.at(pCon->m_Shape_A);
      ...
      if (isVehicle(pCon->m_Shape_B)) {
        boost::shared_ptr<Vehicle_Entity> Shape_B = ray_vehicles_map_.at(pCon->m_Shape_B);
        RigidShape_B = Shape_B->rigid_body_.get();
      }
      else {
        boost::shared_ptr<Entity> Shape_B = shapes_map_.at(pCon->m_Shape_B);
...
void PhysicsEngine::RunDevices() {
  for (unsigned int ii = 0; ii<sim_devices_.size(); ii++) {
    ...
    if (Device->m_sDeviceType == "CarController") {
      ...
      for (std::map<std::string, boost::shared_ptr<Vehicle_Entity> > ::iterator it =
              ray_vehicles_map_.begin(); it!= ray_vehicles_map_.end(); it++) {
        if (pCarCon->GetBodyName() == it->first) {
          //  This controller goes to this car.
          boost::shared_ptr<Vehicle_Entity> eVehicle = it->second;
          VehiclePtr pVeh = eVehicle->vehicle_;
          boost::shared_ptr<SimRaycastVehicle> vehicle_shape =
              boost::dynamic_pointer_cast<SimRaycastVehicle>(
    ...
    /////////////
      ...
      for (std::map<std::string, boost::shared_ptr<Entity> > ::iterator it =
...
std::vector<Eigen::Matrix4d> PhysicsEngine::GetVehicleTransform(
  ...
  boost::shared_ptr<Vehicle_Entity> std_Vehicle =
