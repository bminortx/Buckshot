...
/// bulletInterface.cpp. Any method called from there MUST be in this
/// class.

void bulletSim::InitSimulation(int is_scenegraph_on) {
  bool render_option_ = (is_scenegraph_on == 1) ? true : false;
  scene_.Init(models_, false, render_option_);
}
...
int bulletSim::AddSphere(double radius, double dMass, double dRestitution,
                         double* position, double* rotation) {
...
int bulletSim::AddCylinder(double radius, double height, double dMass,
...
int bulletSim::AddTerrain(int row_count, int col_count, double grad,
...
int bulletSim::AddRaycastVehicle(double* parameters, double* position,
                                 double* rotation) {
