...
class PhysicsEngine {
 public:
  ...
  void RegisterObject(const boost::shared_ptr<ModelNode>& pItem);
  ...
  std::map<std::string, boost::shared_ptr<Vehicle_Entity> >    ray_vehicles_map_;
  std::map<std::string, boost::shared_ptr<Entity> >            shapes_map_;
  std::map<std::string, boost::shared_ptr<Compound_Entity> >   compounds_map_;
  ...
  boost::shared_ptr<btDefaultVehicleRaycaster> vehicle_raycaster_;

 private:
  /// PRIVATE MEMBER VARIABLES
  btDefaultCollisionConfiguration  collision_configuration_;
  boost::shared_ptr<btCollisionDispatcher> bt_dispatcher_;
  boost::shared_ptr<btDbvtBroadphase> bt_broadphase_;
  boost::shared_ptr<btSequentialImpulseConstraintSolver> bt_solver_;
  double timestep_;
  double gravity_acc_;
