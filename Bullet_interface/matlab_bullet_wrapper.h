 ...
 * Author: bminortx
 * This wrapper is designed to facilitate the use of the Bullet Physics simulator
...
// Our entities!
// These hold the Shape_Entity, Compound_Entity, Vehicle_Entity and
...
// Our functions (optimizing)
//#include "../Bullet_functions/speed_run.h"
...
public:
  ...
  int AddCompound(double* Shape_ids, double* Con_ids,
    ...
    pCompound->m_vShape_ids = Shape_ids;
    pCompound->m_vCon_ids = Con_ids;
    if(!std::strcmp(CompoundType, "Vehicle")){
      pCompound->m_Type = VEHICLE;
    }
  ...
  int AddRaycastVehicle(double* parameters, double* position, double* rotation){
  ...
  void CommandVehicle( double id, double steering_angle, double force )
  {
    ...
    double* Shape_ids = Vehicle->m_vShape_ids;
    double* Con_ids = Vehicle->m_vCon_ids;
    ...
    boost_wheel_bl->m_pRigidBody->applyTorque(torque);
    boost_wheel_br->m_pRigidBody->applyTorque(torque);
  }
  ...
  double* GetShapeTransform(double id){
    ...
    btTransform world_transform = boost_Entity->
        m_pRigidBody->getCenterOfMassTransform();
